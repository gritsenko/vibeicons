...
3. If `icons.length === 0`, the main area renders `<LibraryEmpty>` instead of the grid — CTAs that open the catalog modal or the JSON / SVG-folder pickers. Every import is explicit user action; nothing is pulled in automatically unless preset.json flags an entry `auto`.
...
That flat list is rendered by `components/PresetCatalog.tsx` — grouped by `collection`, one row per library with its icon count and size, a per-row **Import** button, and an **Import all**. It appears in two places: `components/CatalogModal.tsx` (the onboarding picker, auto-opened once per session when the app comes up on an empty library, also reachable from the empty-state CTA) and the Settings modal. The picker is deliberately a **modal, not part of the empty state**: the library stops being empty the moment the first pack lands, and the user has to be able to queue up the rest without the picker vanishing under them. `App.tsx::importPresetSources(entries?)` does the work: it skips sources already present in `sources`, feeds the rest through the regular `handleImport` path with a shared `taken` set, and tracks in-flight source names in `presetBusy` so each row spins on its own.
